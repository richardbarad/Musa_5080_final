...
testProbs %>%
  group_by(Outcome) %>% summarise(median_probs=median(probs3)) %>%
  cbind(.,Outcome2=c('Pass','Fail')) %>%
  dplyr::select(Outcome2,median_probs) %>%
  kbl(col.names=c('Outcome','Median Predicted Fail Probability')) %>%
  kable_minimal()
```
