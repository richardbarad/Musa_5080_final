...
set.seed(3456)
trainIndex <- createDataPartition(y = paste(data_2021_2$pri_neigh, data_2021_2$fail), p = .65,list = FALSE,times = 1)
...
testProbs <- data.frame(Outcome = as.factor(test$fail),
...
auc1 <- pROC::auc(testProbs$Outcome,testProbs$probs1)
auc2 <- pROC::auc(testProbs$Outcome,testProbs$probs2)
auc3 <- pROC::auc(testProbs$Outcome,testProbs$probs3)
...
# Looking Senstivity and Specifity by Neighboorhood / (Establishment Type?)

# Look at Model Performance by Neighboorhood Type (i.e: race)
