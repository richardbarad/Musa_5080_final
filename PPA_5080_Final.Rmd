...
```{r examine_means}
# Look at the means for each continuous variable

data_2021_2_means <- data_2021_2 %>%
  st_drop_geometry() %>%
  group_by(results) %>% summarize_at(vars("age","prev_fails","Median_Rent","pct_white","pct_poverty","rodents.nn","sanitation.nn","fails_Per_year"),mean,na.rm=TRUE) %>%
  gather(key='variable',value='mean',-results)

ggplot(data=data_2021_2_means,aes(x=results,y=mean,fill=results))+
