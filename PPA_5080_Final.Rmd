...
```
```{r sanitation_complaints, cache=TRUE}
...
ggplot() + 
  ...
  geom_sf(data = sanitation2, colour="blue", size=0.1, show.legend = "point") +
...
```{r rodents_complaints, cache=TRUE}

rodents <- read.socrata("https://data.cityofchicago.org/resource/v6vf-nfxy.json?SR_TYPE='Rodent%20Baiting/Rat%20Complaint'")
...
ggplot() + 
  geom_sf(data = chicagoboundary) +
  geom_sf(data = rodents2, colour="blue", size=0.1, show.legend = "point") +
...
ggplot() +
  ...
  scale_color_viridis_c(name = "count", trans = "log",breaks = my_breaks, labels = my_breaks)+
...
data_2021_2_means <- data_2021_2 %>%
  st_drop_geometry() %>%
  group_by(results) %>% summarize_at(vars("age","prev_fails","Median_Rent","pct_non_white","pct_poverty","rodents.nn","sanitation.nn","fails_Per_year"),mean,na.rm=TRUE) 
...
data_2021_2_means %>%
  ...
  scale_fill_manual(values=c('orange','lightgreen'),name='Pass Inspection')+
...
p_values = c(t_age,t_fails_per_year,t_Median_Rent,t_pct_non_white,t_pct_poverty,t_prev_fails,t_rodents.nn,t_sanitation.nn)
...
data_2021_2 %>% dplyr::select(age,prev_fails,Median_Rent,pct_non_white,pct_poverty,rodents.nn,sanitation.nn,fails_Per_year) %>%
  st_drop_geometry() %>%
  correlate() %>% 
  autoplot() +
