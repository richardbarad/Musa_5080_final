...
clean_inspections <- function(df,y){
  ...
  st_transform('ESRI:102271') %>%
  dplyr::filter(year == y & results != 'Out of Business' & results != 'No Entry' & results != 'Not Ready') %>%
  ...
  dplyr::filter(address != '5700 S CICERO AVE')
  return(clean_df)
}
...
If restaurant is not located inside a census tract, set census data equal to mean of the neighboorhoods the restaurant is in.
...
buffer <- st_buffer(data_2021_2,500)

rest_500m <- st_join(data_2021_2 %>% dplyr::select('geometry'),buffer %>% select('inspection_id')) %>%
  st_drop_geometry() %>%
  group_by(inspection_id) %>% tally() %>%
  ungroup() %>%
  rename('res_500m' = n)
...
data_2021_2 <- data_2021_2 %>%
    mutate(rodents.nn = nn_function(st_coordinates(data_2021_2),st_coordinates(rodents2),k = 50),
          ...
          prev_fail.nn = nn_function(st_coordinates(data_2021_2),st_coordinates(fail_2020),k=10)) %>%
  left_join(.,rest_500m, by = 'inspection_id') %>%
  mutate(prev_fail_norm3 = prev_fail.nn / res_500m)
```
...
quant_predictors <- c("rodents.nn","sanitation.nn","age","prev_fails","Median_Rent","pct_non_white","pct_poverty","fails_Per_year","thefts.nn","prev_fail_norm3")
...
# Alec Is Working On This - Not Running/Complete Yet

```{r}

# Split Data Into 65:35 Training Dataset and Test Dataset
...
``` {r model2}
model2 <- glm(fail ~ age + prev_fails + Median_Rent + rodents.nn + sanitation.nn + thefts.nn + pct_poverty + prev_fail_norm3,
                     data = data_2021_2,
                     family="binomial" (link="logit"))
...
``` {r model3}
model3 <- glm(fail ~ age + prev_fails + Median_Rent + log_rodents.nn + log_sanitation.nn + thefts.nn + risk + pri_neigh + log_pct_poverty + prev_fail_norm3,
                     data = data_2021_2,
                     family="binomial" (link="logit"))
...
# Making Predictions - Alec Is Working On This Code, Incomplete

```{r}

predictions <- data.frame(Outcome = as.factor(#TestSet$#Unsure),
                          Probability = predict(model03, #TestSet, type="response"))
```

# Sensitivity and Specificity - Alec Is Working On This Code, Incomplete

```{r}

ggplot(predictions, aes(x = Probability, fill = as.factor(Outcome))) + 
  geom_density() +
  facet_grid(Outcome ~ .) +
  scale_fill_manual(values = palette) +
  labs(x = "Rename", y = "Rename",
       title = "Rename") +
